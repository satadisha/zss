...
with open("regular_parser.txt",encoding="utf-8") as f1:
    ...
    for tree in trees:
        ...
        tree_holders.append(list(filter(None,tree.split("\n\n"))))
...
for trees in tree_holders:
    ...
    for tree in trees:
        ...
        i=0
        sentences=list(filter(None,tree.split("\n")))
        for sentence in sentences:
            if len(sentence)>1:
                b=list(filter(None,sentence.split("\t")))
                b[3]= lookup(b[3])
                b[4]=b[3]
                b_holder.append(b) 
                if(("'" in b[1])& (b[3]!="NNP")):
...
'''file_to_write=open("stanford_processed_input.txt","w",encoding="utf-8")
str1=""
